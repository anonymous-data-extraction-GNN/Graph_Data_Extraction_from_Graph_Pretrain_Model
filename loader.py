...
class MoleculeDatasetForAttack(InMemoryDataset):
    def __init__(self,
                 root,
                 #data = None,
                 #slices = None,
        ...
        if not empty:
            self.data, self.anchor_nodes, self.slices = torch.load(self.processed_paths[0])
    ...
    def get(self, idx):
        ...
        for key in self.data.keys:
            ...
            s[data.cat_dim(key, item)] = slice(slices[idx],
                                                    slices[idx + 1])
            data[key] = item[s]
        return data, self.anchor_nodes[idx]
