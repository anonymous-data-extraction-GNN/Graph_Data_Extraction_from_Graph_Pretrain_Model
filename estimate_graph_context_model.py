...
def train(args, model_substruct, model_context, loader, optimizer_context, device):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        substruct_rep = model_substruct(batch.x_substruct, batch.edge_index_substruct, batch.edge_attr_substruct)[batch.center_substruct_idx].detach()
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default = 'zinc_standard_agent_pretrain', help='root directory of dataset for pretraining')
    parser.add_argument('--pretrained_model_file', type=str, default='saved/pretrain.pth', help='filename of the pretrained model')
    parser.add_argument('--output_model_file', type=str, default = 'saved/estimated.pth', help='filename to output the model')
    ...
    state = torch.load(args.pretrained_model_file)
    model_substruct.load_state_dict(state['model_substruct'])
    
    # model_substruct.load_state_dict(state[])

    for epoch in range(1, args.epochs+1):
        print("====epoch " + str(epoch))
        
        train_loss, train_acc = train(args, model_substruct, model_context, loader, optimizer_context, device)
        print(train_loss, train_acc)

    if not args.output_model_file == "":
        torch.save(model_substruct.state_dict(), args.output_model_file)
